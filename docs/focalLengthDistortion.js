...
runButton.addEventListener("click", async () => {
    ...
    try {
        ...
        const k1modelpath = `models/${focalLength}mm_fl_k1.onnx`;
        const k2modelpath = `models/${focalLength}mm_fl_k2.onnx`;
        ...
        const modeimgpath = `models/${focalLength}mm_fl.png`;
        const img = document.createElement("img");
        img.src = modeimgpath;
        img.style.width = "100%";
        outputElement.appendChild(img);
    } catch (error) {
        // Append errors to the output element
